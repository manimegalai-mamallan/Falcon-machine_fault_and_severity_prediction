...
class FaultPredictor:
    ...
    def _reorder_severity_probs(self, sev_raw: np.ndarray, model) -> np.ndarray:
        ...
        target = list(shared_config.SEVERITY_LEVELS)
        if not hasattr(model, "classes_") or model.classes_ is None:
            return sev_raw
        model_order = list(model.classes_)
        if model_order == target:
            return sev_raw
        # Reorder columns
        out = np.zeros_like(sev_raw)
        for i, level in enumerate(target):
            if level in model_order:
                out[:, i] = sev_raw[:, model_order.index(level)]
        return out
